Algoritmo Sistema_Ventas_Perfumes
    ...
    Definir nombre_ingresado, fecha_pedido, nombre_cliente_venta, termino_busqueda, dni_ingresado Como Caracter
    Definir total_pagar Como Real
    Definir encontrado, venta_encontrada, cliente_existe Como Logico
	Definir dni_repetido Como Logico
    
    // ARREGLOS DE INVENTARIO (Capacidad 100)
    Dimension codigos[100]
    Dimension nombres_perfumes[100]
    Dimension stock[100]
	...
	Dimension dni_cli[100]
	Dimension nombres_cli[100]
    total_clientes <- 0
    
    // =========================================================
    // NUEVOS ARREGLOS: HISTORIAL DE VENTAS (Capacidad 100)
    // =========================================================
    Dimension historial_cliente[100]
    Dimension historial_perfume[100]
    Dimension historial_cantidad[100]
    Dimension historial_total[100]
    Dimension historial_fecha[100]
	Dimension historial_dni[100]
    total_ventas <- 0
    ...
    Mientras opcion_menu <> 6 Hacer
        ...
        Si opcion_menu = 1 Entonces
			...
			Mientras Longitud(dni_ingresado) <> 8 Y Longitud(dni_ingresado) <> 12 Hacer
				Escribir "ERROR: El documento debe tener 8 o 12 digitos."
				Escribir "Ingrese nuevamente el documento: "
				Leer dni_ingresado
			Fin Mientras
			...
			Si total_clientes > 0 Entonces
				Para j <- 1 Hasta total_clientes Hacer
					Si dni_cli[j] = dni_ingresado Entonces
						cliente_existe <- Verdadero
						nombre_cliente_venta <- nombres_cli[j]
					Fin Si
				Fin Para
			Fin Si
			...
			Para i <- 1 Hasta total_productos Hacer
				Escribir i, ". ", nombres_perfumes[i], " | Stock: ", stock[i], " | Precio: S/ ", precios[i]
			Fin Para
			
			Escribir "-------------------------"
            Escribir "Ingrese el NOMBRE del perfume buscado(Ej. Stronger with You Intensely): "
            ...
            SiNo
				Si opcion_menu = 3 Entonces
					...
					Si sub_opcion = 1 Entonces
						...
						Si dni_repetido = Falso Entonces
							Si total_clientes < 100 Entonces
								total_clientes <- total_clientes + 1
								nombres_cli[total_clientes] <- nombre_cliente_venta
								dni_cli[total_clientes] <- dni_ingresado
							...
							Sino
								Escribir "ERROR: Directorio lleno."
							Fin Si
						Sino
					...
					Sino
						...
						Sino
							...
							Para j <- 1 Hasta total_clientes Hacer
								Escribir j, ". ", nombres_cli[j], " - DNI: ", dni_cli[j]
							Fin Para
						Fin Si
					Fin Si
				SiNo
                    ...
                    SiNo
                        Si opcion_menu = 5 Entonces
                            ...
                            Si tipo_busqueda = 1 Entonces
								Escribir "Ingrese el DNI del cliente:"
							Sino
								Escribir "Ingrese el nombre del perfume:"
							Fin Si
                            ...
                            Para k <- 1 Hasta total_ventas Hacer
                                Si (tipo_busqueda = 1 Y historial_dni[k] = termino_busqueda) O (tipo_busqueda = 2 Y historial_perfume[k] = termino_busqueda) Entonces
                                    Escribir "-----------------------------------"
                                    Escribir "Comprador: ", historial_cliente[k]
									Escribir "DNI      : ", historial_dni[k]
                            ...
                            Si venta_encontrada = Falso Entonces
                                Escribir "No se encontraron registros de ventas."
                                Escribir "-----------------------------------"
                            Fin Si
